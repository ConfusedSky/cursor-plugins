...
3. Migrate one component at a time. Each is an independent artifact, so parallelize across worktrees, one owner per component; two owners never write the same worktree or branch. Shared primitives migrate first as a blocking phase, so parallel owners consume them instead of racing to edit them.
